...
namespace QuantConnect.DataSource.MetaTrader.QuantConnect.DataSource.MetaTrader.MetaTrader
{
    ...
    /// </summary>
    public class MetaNewsProvider : IDataQueueHandler
    {
        private readonly List<Symbol> _symbolList = new List<Symbol>();
        private readonly bool _filterTicks;
        private HashSet<NewsItem> _emittedNews = new HashSet<NewsItem>();
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="MetaNewsProvider"/> class.
        /// </summary>
        public MetaNewsProvider()
        {
        ...
        private void GetLatestNews(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var lastHourNews = JsonConvert.DeserializeObject<List<NewsItem>>(content)
                    .Where(n => utcNow - n.CrawlDate <= TimeSpan.FromHours(1));

                _emittedNews = new HashSet<NewsItem>(_emittedNews.Where(n => utcNow - n.CrawlDate < TimeSpan.FromHours(3)), _comparer);
                ...
                lock (_locker)
                {
                    ...
                    foreach (var tiingoNews in lastHourNews.Where(n => _emittedNews.Add(n)))
                        foreach (var tiingoNewsSymbol in tiingoNews.Symbols)
                        {
                            ...
                            tiingoNews.Symbol = Symbol.CreateBase(typeof(NewsItem), tiingoNewsSymbol, Market.USA);
    ...
    internal class TiingoNewsComparer : IEqualityComparer<NewsItem>
    {
        /// <summary>
        /// Check equality.
        /// </summary>
        ...
        /// <returns></returns>
        public bool Equals(NewsItem thisOne, NewsItem anotherOne)
        {
